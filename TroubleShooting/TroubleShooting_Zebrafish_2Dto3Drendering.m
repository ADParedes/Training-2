...
valsBFBF={};
for i=1:znum
    I               = valsGFP{1}{2}{1}{i};% parts,position,frame,znum
    I_BF            = valsBF{1}{2}{1}{i};
    I2              = im2uint8(I);
...
hf1=figure();
    for i=1:n
        ...
        ZZ = i * ones(rows,cols);
        surf(XX,YY,ZZ,Dbf(:,:,1),'edgecolor','none')
        colormap('gray')
        hold on;
    end;
...
%%
bwImage=cat(n,bw_color_MPEG,bw_color_MPEG,bw_color_MPEG,bw_color_MPEG,bw_color_MPEG,bw_color_MPEG,bw_color_MPEG); %Creates 512x512xn, n=zstack
size(M);
...
hf2=figure();
    for i=1:n
        ...
        [XX,YY]= meshgrid((1:cols),(1:rows));
        ZZ = i * ones(rows,cols);
        surf(XX,YY,ZZ,bwImage(:,:,1),'edgecolor','none')
        colormap('spring')
        hold on;
    end;
