...
%NOTE:  nanmean([ ]) will average ignoring nan values!!!! Good find.  
disp('Redo WoundRegion')
disp('If Removing Movement remember to save handles')
disp('Last Update: 2017-10-11')
pause(2);

%% Parameters
ParameterA              = 0.70; %Trackability per SM a.k.a MajorityTracksPercent=.70; %<--HIGH% selection - ADP
Parameter_gtA           = 0.9;  %Trackability per GT
ParameterB              = 1;    %StaticLimit (1 provides best result)  i.e. 0.8519um. 
ParameterS              = 150;  %Spatial Interval (150um)
lateralpixelresolution  = 1.64; %Specific to UIC RRC MrBig Confocal Microscope
ZstepMicrons            = 10;   %Selected this during imaging -
samplingfrequency       = ti_d; %minutes

disp('chose zstack based on BF/Phase Contrast..not always accurate');
...
% 
% elseif handles.rows==256
%     micronsPerPixel=3.102;
% else
%     micronsPerPixel=1.64; %this is for confocol microscopy in WEST CAMPUS RRC
%     ZstepMicrons=10; 
% end;
micronsPerPixel=1.64; %this is for confocol microscopy in WEST CAMPUS RRC  %
ZstepMicrons=10;  %
...
%Typically 22 microns therefore that is how many pixels

ZstepPixel=ZstepMicrons/micronsPerPixel;
...
GT_45=round((45-t_plate)/ti_d); %%%%%%%%%%%%%%%%%%%%CHANGING
GT_45=3;
...
arr_Time                = round((arr_T(1:frames)*ti_d+t_plate)-ti_d)';
disp('End: Times')
%% GT in wound Count
gtinwoundcount=0;
beat=0;
...
CTTM=(ti_d*handles.numFrames)+tplate;%Corrected_Total_Time_Max
...
Hnn=handles.nodeNetwork;   %Node Network ()
[m,n]=size(Hnn);
[~,mp]=size(handles.finalNetwork); %Num_TrackMax is  the same as mp
...
disp('End Phagosight Frame and Volume')
...
%--Loop
Forward                     = 0; %structure array
Backward                    = 0; %structure array
FtoB                        = 0; %structure array
FBratio                     = 0; %structure array
WoundScoreUm                = 0; %structure array
WoundScore1234              = 0; %structure array
WoundStartUm                = 0; %structure array
WoundDist                   = 0;
     ...
     if arr_boo_GT(numGT) %ensures that these frames exist
        ...
        for numTrack=S_iterations %This specific 'Selected group' TRACK 
            ...
            %This would happen only when we do 45 because it might not exist    
                ...
                elseif (dist_notochord+150) < startwound_um_interval(numTrack)&& startwound_um_interval(numTrack) <=(dist_notochord+ParameterS+ParameterS) 
                     wound1234_interval(numTrack)              = 3;
                elseif (dist_notochord+150+150) < startwound_um_interval(numTrack)&& startwound_um_interval(numTrack) <=(dist_notochord+ParameterS+ParameterS+ParameterS) 
                ...
                elseif dist_notochord < startwound_um_cum(numTrack)&& startwound_um_cum(numTrack) <=(dist_notochord+150)
                    wound1234_cum(numTrack)              = 2;
                elseif (dist_notochord+150) < startwound_um_cum(numTrack) && startwound_um_cum(numTrack) <=(dist_notochord+150+150) 
                     wound1234_cum(numTrack)              = 3;
                elseif (dist_notochord+150+150) < startwound_um_cum(numTrack) && startwound_um_cum(numTrack)<=(dist_notochord+150+150+150) 
