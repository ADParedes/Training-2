...
%         B(B<1)=0;B=~B;B=~B;  
        ...
        c40           = mask_combo & bw;
        ixelNum(i)       = sum(sum(c40));%size(OriginalImgBF,1)*size(OriginalImgBF,2)-CellPixelNum; % or sum(sum(~CellMask))02/
        ixelIntensity = sum(sum(uint16(c40).*I_TR))/ixelNum(i);
        MeanFluorescence(i)            = ixelIntensity;
        %% 
        %-PreSet
        OriginalImgTR               = I_TR1;
        RosRegionMask               = RR;
...
%% CTCF No wound
nowound_IntDen  = (ixelNum).* MeanFluorescence; %Integrated Density
nowound_CTCF            = nowound_IntDen - ((ixelNum).*MeanBgs);%Corrected Total Cell Fluorescence 
