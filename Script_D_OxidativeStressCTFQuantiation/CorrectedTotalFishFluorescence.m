...
disp(strcat('40 Circles: ',num2str(PreMF)))
disp('END: Part 1: PreInjury ROS Calculation')
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
%% Part 2: Quantitate ROS in Region of Injury after Injury
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
...
len_dname       = length(dname5);
%-single values
Time            = 0; %Corresponding time to frame
timepoints      = 0; % selected time points for editing
T_p             = 0; % respective frames correspoinding to 'timepoints'
tp              = 0;
%-Cell Arrays
Arr_Bkgd        ={0};
Arr_Ifish       ={0};
...
disp('Processing...')
disp(name5)
%-Determining the Time for the loop
Time                = round((arr_T(1:frames)*ti_d+t_plate)-ti_d);
...
T_p=cat(2,(1),tp);
%-More Precurors for the loop
iteration       = 200;
everyframe      = (1:frames);
everyiteration  = (1:iteration:frames);
golditeration   = cat(2,T_p,everyiteration);
% golditeration   = (1:5:frames); %This is the way to change everother
Threshold       = 7000;
...
%     elseif ~isempty(find(i==everyiteration))
   ...
   %% Archived ROI spatial references
    ...
    for j=1:length(f_TexRed)
        ...
        every20  = (1:20:frames);
        if  ~isempty(find(i==golditeration))%i==1
            diameter                = 60; % approximately 36 um on this confocal setting 
            [mmp_I bwfinal ]        = mindthegap(I_TR,bw_color_realsegout,bw_fish,diameter);
            mfi(i)                  = mmp_I;
            BWFINAL{i}              = bwfinal;
            BWFISH{i}               = bw_fish;
        else
        ...
        %% CTCF Calculation
         ...
         CTCF(i)   = IntDen(i) - (AreaROI(i)*MeanBgs(i));%Corrected Total Cell Fluorescence 
        %%
        %-PreSet
    ...
    %-Finalize Array Savings
        ...
        Bg(i,1)                   = arr_Bkgd(z_i); %Reformated arr version
        Df(i,1)                   = arr_Diff(z_i);%Reformated arr version
        Rw(i,1)                   = arr_Raw(z_i);%Reformated arr version
...
%-
% CTCF_baseline = IntDen - (AreaROI.*Andre);
% mfi
...
%% Did you do a baseline-i.e. NO Wound first
%-no wound will spit out a mmpi0 value
% if one does not exist, to keep script working, we will need to subsittue
% this value - here we substitute it with the PreInjury Background
% subtrated value
lens     = length(PixelNum);
if exist('mmpi0')
else
   mmpi0    = ones(1,lens)*PreDf; 
end;
wound_CTCF            = wound_IntDen - ((PixelNum).*mmpi0);%Corrected Total Cell Fluorescence 
close all
plot(wound_CTCF);
% wound_IntDen  = (PixelNum(1:115)).* mfi(1:115); %Integrated Density
% wound_CTCF            = wound_IntDen - ((PixelNum(1:115)).*mmpi0);%Corrected Total Cell Fluorescence 
% canvas          = imoverlay(I_TR,mask_combo,[1 0 0]);
% mmpi0=MeanFluorescence;
% figure();imshow(canvas);
%% Saving AB0#__WoundStatus
...
elseif editingscript==1
    OutSeg=(0);
    disp('Nothing Saved because you are editing the script')
    for K=1:lens
