...
%     title('Interval')
    ylabel('Meandering Ratio','fontsize',18,'fontweight','bold')
...
disp([h p]); pstat(3)=p;
%intervalSM
[h,p,stats]    =ttest(yyy,yyy1,'Tail','both'); %needs to be the same number of elements?!! what    
disp([h p]); pstat(4)=p;
[h,p,stats]    =ttest(yyy,yyy2,'Tail','both'); %needs to be the same number of elements?!! what    
disp([h p]); pstat(5)=p;
...
disp([h p]); pstat(9)=p;
[h,p,stats]    =ttest(yyy,z,'Tail','both'); %needs to be the same number of elements?!! what    
disp([h p]); pstat(10)=p;
...
% [h,p,stats]    =ttest2(yy,z); %needs to be the same number of elements?!! what    
% disp([h p])
...
% Stats = mwwtest(zy,yyy1); % good
% 
% Stats = mwwtest(zy1,z); % good
% % Stats = mwwtest(zy2,z); % good
% Stats = mwwtest(yy,z); % good
% Stats = mwwtest(yy1,z); % good
% Stats = mwwtest(yy2,z); % good
% %---
% Stats = mwwtest(zy,yy); % good
% Stats = mwwtest(zy,yy1); % good
% Stats = mwwtest(zy,yy2); % good
% %--
% Stats = mwwtest(zy1,yy); % good
% Stats = mwwtest(zy1,yy1); % good
% Stats = mwwtest(zy1,yy2); % good
% %--
% Stats = mwwtest(zy2,yy); % good
% Stats = mwwtest(zy2,yy1); % good
% Stats = mwwtest(zy2,yy2); % good
